...
const UserDetailsBord = ({info,index}) => {
  const {blood_group,city,name,state,phone_number} = info;
  const messageBody = "Blood needed sir.";
  return (
    <>
      <td className="border py-2 w-1/12 text-center cursor-pointer">{index}</td>
      <td className="border py-2 w-2/12 text-center cursor-pointer">{name}</td>
      <td className="border py-2 w-2/12 text-center cursor-pointer">{blood_group}</td>
      <td className="border py-2 w-2/12 text-center cursor-pointer">{city}</td>
      <td className="border py-2 w-2/12 text-center cursor-pointer">{state}</td>
      <td className="border py-2 w-3/12 text-center" onClick={(e)=>e.stopPropagation()}>
