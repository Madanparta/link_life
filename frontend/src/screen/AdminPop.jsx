...
const AdminPop = ({setPopshow,userInfo,donor,bloodbanker}) => {
    ...
    const {name,age,blood_group,city,district,email,state,phone_number,role,_id} = userInfo;
    const {donationQuantity,donationDate} = donorData;
    const {description,quantityAvailable,updatedAt} = bankerData;
    ...
    <>
      ...
      <div className="capitalize">
        <div className="flex flex-col gap-1 overflow-hidden">
            ...
            <p>blood group : <mark>{blood_group}</mark></p>
        ...
        <div className="flex justify-around items-center mt-5">
            <button className="bg-green-400 text-white px-2 py-1 rounded-md hover:bg-green-500 active:bg-green-600 shadow-md">approve</button>
            <button className="bg-red-400 text-white px-2 py-1 rounded-md hover:bg-red-500 active:bg-red-600 shadow-md">reject</button>
        </div>
      </div>
    </>
  )
